...
Variable prog: Ditiospec.program. 
Variable tprog: Cminor.program.
Hypothesis TRANSL: Ditiotrans.transl_program prog = OK tprog.
...
Lemma function_ptr_translated:
  forall (b: block) (fd: Ditiospec.fundef),
...
Inductive match_states: Ditiospec.state -> Cminor.state -> Prop :=
  | match_state:
    forall f c om os rid roff rval rtime m tf ts tk tsp te tm tb tm'
      ...
      (MRTIME: Some (Vint rtime) = te!regaccess_time)
      (TF: Ditiotrans.transl_function f = OK tf)
      (TC: Ditiotrans.transl_code c = OK ts)
      ...
      (MINJ: mem_inj m tm'),
      match_states (Ditiospec.State f c om os rid roff rval rtime m)
  ...
  | match_callstate:
    forall fd om os r m tfd targs tk tm
      ...
      (MK: call_cont tk = Kstop),
      match_states (Ditiospec.Callstate fd om os r m)
                   (Cminor.Callstate tfd targs tk tm)
  | match_returnstate:
    forall a tv tk tm m
      (MV: Vint (Ditiospec.action_enc a) = tv)
      (MINJ: mem_inj m tm)
      (MK: tk = Kstop),
      match_states (Ditiospec.Returnstate a)
                   (Cminor.Returnstate tv tk tm)
  .

Lemma transl_initial_states:
  forall S, Ditiospec.initial_state prog S ->
...
Lemma transl_final_states:
  forall S R r,
  match_states S R -> Ditiospec.final_state S r -> Cminor.final_state R r.
Proof.
  intros.
  inv H0.
  inv H.
...
Proof.
      ...
      auto.
Qed.
Lemma my_expand_not: forall (P Q R S: Prop), ~(P /\ Q /\ R /\ S) -> ~P \/ ~Q \/ ~R \/ ~S.
Proof.
      intros.
      apply Classical_Prop.not_and_or in H.
      destruct H.
...
Lemma transl_step:
  forall S1 t S2, Ditiospec.step ge S1 t S2 ->
...
(** 1- exec_reject step **)
  ...
  - monadInv TC.
    ...
    unfold master_eval_reject in He3.
    inversion He3.
    ...
    unfold master_eval_reject in He3.
      ...
      eapply step_ifthenelse with (v:=Val.of_optbool (Val.cmpu_bool (Mem.valid_pointer tm) Cgt (Vint rtime) (Vint (period2 i)))).

      apply eval_Ebinop with (v1:=Vint rtime) (v2:=Vint (period2 i)). constructor. auto. constructor. constructor.
      unfold Val.cmpu. unfold Val.of_optbool. unfold Val.cmpu_bool. simpl. auto. (* rewrite H; constructor. *)
      unfold Val.cmpu. unfold Val.of_optbool. unfold Val.cmpu_bool.
      unfold Val.cmp. unfold Val.of_optbool. unfold Val.cmp_bool.
      assert (Int.cmpu Cgt rtime (period2 i) = false) as EQ3. assumption.
      rewrite EQ3.
      constructor.
     ...
     apply eval_Ebinop with (v1:=Vint (rtime)) (v2:=Vint (period1 i)). constructor. auto. constructor. constructor.
     simpl. unfold Val.cmpu. unfold Val.of_optbool. unfold Val.cmp_bool.  simpl.
     assert (Int.ltu rtime (period1 i)  = false) as EQ3.
     simpl in He2.
     assumption.
     rewrite EQ3. reflexivity. constructor.
     (****)
     eapply star_step; [ idtac | idtac | auto ].
     eapply step_ifthenelse with (v:=Vtrue).
     apply eval_Ebinop with (v1:=Vint rid) (v2:=Vint (Int.repr(dev_id (master_dev i)))).
     constructor. auto. constructor. auto. simpl. unfold Val.cmp. unfold Val.of_optbool. simpl.
     assert (Int.eq rid (Int.repr (dev_id (master_dev i)) )  = true) as EQ3.
     rewrite <- H1.
     apply Int.eq_true.
     rewrite EQ3. reflexivity. constructor.
     eapply star_step; [ idtac | idtac | auto ].
     ...
     eapply step_ifthenelse with (v:=Vtrue).
      ...
      apply L_will_dev_go_target_state with (o:=om) (rid:=rid) (roff:=roff) (rval:=rval)
                                            (m:=m) (tm':=tm') (tf:=tf) (iomem_p:=m_iomem_p); repeat(assumption).
...
(** 2- exec_skip step **)
  ...
  - monadInv TC.
    ...
    apply my_expand_not in H1.
    Lemma my_expand_or: forall P Q R S, P \/ Q \/ R \/ S -> P \/ (~P /\ Q) \/ (~ P /\ ~Q /\ R) \/ (~ P /\ ~Q /\ ~R /\ S).
    Proof.
    intros.
    
    destruct (classic P) as [p | pn].
    + left; auto.
    + right. destruct (classic Q) as [q | qn].
    - left; auto.
    - destruct (classic R) as [r | rn].
    right. left; auto.
    destruct (classic S) as [s | sn].
    right. right; auto.
    crush.
    Qed.

    apply my_expand_or in H1.
    destruct H1.
    apply remove_OK in EQ1.
      ...
      apply eval_Ebinop with (v1:=Vint rtime) (v2:=Vint (period2 i)). constructor. auto. constructor.
      auto. simpl. unfold Val.cmp. unfold Val.of_optbool. unfold Val.cmp_bool. simpl.

      assert (Int.ltu (period2 i) rtime = true) as EQ3.
      Lemma my_false_true: forall P, (P <> false) -> (P = true).
      Proof.
      intros.
      destruct P; auto.
      Qed.
      apply my_false_true in H1; assumption.
      
      rewrite EQ3.
      unfold Val.cmpu.
      unfold Val.cmpu_bool.
      apply my_false_true in H1.
      rewrite H1; auto.

        unfold Val.of_optbool. unfold Val.cmpu_bool. simpl.
        apply my_false_true in H1.
        simpl in H1.
      ...
      eapply star_step; [ idtac | idtac | auto ]. constructor. constructor. auto.
      ...
      inversion EQ1.
      unfold C_return_allow. unfold C_return_allow in H2.
      econstructor; split.
      eapply plus_left with (t1:=E0) (t2:=E0); [ constructor | idtac | auto ].
      eapply star_step; [ idtac | idtac | erewrite E0_left; auto ].
      ...
      assert (Int.ltu (period2 i) rtime = false) as EQ3.
      simpl in H3.
      Lemma my_apply_not: forall P, (~ P <> false) -> (P = false).
      Proof.
      intros.
      destruct P; auto.
      crush.
      Qed.
      apply my_apply_not in H3; auto.

      rewrite EQ3.
      unfold Val.cmpu. unfold Val.of_optbool. unfold Val.cmpu_bool. simpl.
      rewrite EQ3; auto.
      (*****)
      unfold Val.cmpu. unfold Val.of_optbool. unfold Val.cmpu_bool. simpl. auto.  
      assert (Int.ltu (period2 i) rtime = false) as EQ3.
      inversion H1.
      apply my_apply_not in H3.
      auto.
      rewrite EQ3; constructor.

      (**** skip to else****)

      eapply star_step; [ idtac | idtac | auto ].
      eapply step_ifthenelse with (v:=Val.of_optbool (Val.cmpu_bool (Mem.valid_pointer tm) Clt (Vint rtime) (Vint (period1 i)))).
      unfold Val.cmpu. unfold Val.of_optbool. unfold Val.cmpu_bool. simpl. auto.

      apply eval_Ebinop with (v1:=Vint rtime) (v2:=Vint (period1 i)). constructor. auto. constructor.
      auto. simpl. unfold Val.cmp. unfold Val.of_optbool. unfold Val.cmp_bool. simpl.

      inversion H1. 

      assert (Int.ltu rtime (period1 i) = true) as EQ3.
      apply my_false_true in H4.
      simpl in H4; auto.
      rewrite EQ3. 
      unfold Val.cmpu. unfold Val.of_optbool. unfold Val.cmpu_bool. simpl.
      rewrite EQ3; auto.
      ...
      apply eval_Ebinop with (v1:=Vint rid) (v2:=Vint (Int.repr (dev_id (master_dev i)))). constructor. auto. constructor. constructor.
      unfold Val.cmpu. unfold Val.of_optbool. unfold Val.cmpu_bool. simpl. auto.
      unfold Val.cmpu. unfold Val.of_optbool. unfold Val.cmpu_bool.
      ...
      apply not_not_equal in H7.
      rewrite <- H7.
      (**)
      unfold master_eval_reject in H8.
      unfold C_eval_master_access.
      apply Classical_Prop.not_and_or in H8.
      
      Lemma my_expand_or2: forall P Q, P \/ Q -> P \/ (~P /\ Q).
      Proof.
      intros.
      apply my_or_in_assumption. assumption.
      Qed.
      
      apply my_expand_or2 in H8.
      auto.      
      
      eapply step_ifthenelse with (v:=Vtrue).
      apply eval_Ebinop with (v1:=Vint rid) (v2:=Vint rid). constructor. auto. constructor. constructor.
      simpl. unfold Val.cmp. unfold Val.of_optbool. unfold Val.cmp_bool. simpl.
      assert (Int.eq rid rid = true) as EQ3.
      apply Int.eq_true. rewrite EQ3. auto. constructor. 
      
      inversion H1. inversion H4. unfold master_eval_reject in H6.
      inversion H6. 
      apply Classical_Prop.not_and_or in H8.
      apply my_expand_or2 in H8.
      destruct H8.
      ...
      apply L_NOT_will_dev_go_target_state with (o:=om) (rid:=rid) (roff:=roff) (rval:=rval)
                                            (m:=m) (tm':=tm') (tf:=tf) (iomem_p:=m_iomem_p); eauto.
      
      constructor.
      
      eapply star_step; [ idtac | idtac | auto ].
      unfold C_return_reject. constructor. constructor. auto. auto.
...
(* 4th (rtime) *)
    ...
    eapply star_step with (t1:=E0) (t2:=E0); [ constructor; constructor; constructor | idtac | auto ].
    eapply star_step with (t1:=E0) (t2:=E0). constructor.
    intros; apply eval_Eload with (vaddr:=Vptr (r) (Int.repr 3)).

    apply eval_Ebinop with (v1:=Vptr r Int.zero) (v2:=Vint (Int.repr 3)).
           constructor. simpl. eauto. 
    apply eval_Econst. unfold eval_constant. auto. auto.
    unfold Mem.loadv.
    eauto.
    ...
    eapply star_step with (t1:=E0) (t2:=E0); [ constructor; constructor; constructor | idtac | auto ].
    constructor. constructor. constructor. constructor. constructor. econstructor; eauto.  
...
Theorem transl_program_correct:
    forward_simulation (Ditio.semantics prog) (vCminorp.semantics tprog).
Proof.
