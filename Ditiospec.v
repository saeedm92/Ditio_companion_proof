...
Notation REGACCESS_TIME_OFFSET := 3%Z.
...
Inductive state : Type :=
  | State:
    forall (f: function)         (**r function *)
           ...
           (rtime: int)          (**r register access: register access time *)
...
Definition trans_uni_r_reject (i: invariant) (om os: block) (rid roff rval rtime: int) (m: mem) :=
   (Int.cmpu Cgt rtime (i.(period2)) = false) /\ (Int.cmpu Clt rtime (i.(period1)) = false) /\ (rid = Int.repr (i.(master_dev)).(dev_id)) /\ (master_eval_reject i om os rid roff rval m).


Definition eval_inv_reject (i: invariant) (om os: block) (rid roff rval rtime: int) (m: mem) :=
  i.(inv_binder) = uni_r /\
  trans_uni_r_reject i om os rid roff rval rtime m.

Inductive step (ge: genv) : state -> trace -> state -> Prop :=
  | exec_reject:
    forall f c i om os rid roff rval rtime m,
    eval_inv_reject i om os rid roff rval rtime m ->
    step ge (State f (i :: c) om os rid roff rval rtime m)
  ...
  | exec_skip:
    ...
    ~ eval_inv_reject i om os rid roff rval rtime m ->
    step ge (State f (i :: c) om os rid roff rval rtime m)
         E0 (State f c om os rid roff rval rtime m)

  | exec_allow:
    forall f om os rid roff rval rtime m,
    step ge (State f [] om os rid roff rval rtime m)
         E0 (Returnstate A_allow)
  | exec_call:
    forall  f om os r rid roff rval rtime m,
    ...
    Mem.load Mint8unsigned m r REGACCESS_TIME_OFFSET = Some (Vint rtime) ->
    step ge (Callstate (Internal f) om os r m)
         E0 (State f f om os rid roff rval rtime m).

Inductive initial_state (p: program): state -> Prop :=
  | initial_state_intro: forall b fd m0 m1 m2 m3 m4 m5 m6 m_iomem s_iomem reg rid roff rval rtime,
    ...
    Mem.load Mint8unsigned (fst (Mem.alloc m6 0
        ...
        | Z.pos y' => Z.pos y'~0~0
        | Z.neg y' => Z.neg y'~0~0
        end)) reg (Int.unsigned (Int.repr 3)) = Some (Vint rtime) ->
