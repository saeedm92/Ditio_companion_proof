...
Definition regaccess_time : ident := 5%positive.
Definition m_iomem_p : ident := 6%positive.
Definition s_iomem_p : ident := 7%positive.
...
Definition C_is_tran_dst_not_inv_ss (tran : state_trans) (inv_ss : sstate) : Cminor.expr :=
  ...
  let tran_dst_regvar_mask := Econst (Ointconst (Int.repr (((tran.(dst)).(ss_regvar)).(rv_mask)))) in
  Ebinop Oand (Ebinop Oand (Ebinop (Ocmp Ceq) tran_dst_reg_off inv_ss_reg_off)
                           (Ebinop (Ocmp Cne) tran_dst_val inv_ss_val)) (* Note: The diff is in this line *)
              (Ebinop Oand (Ebinop (Ocmp Ceq) tran_dst_regvar_off inv_ss_regvar_off)
                           (Ebinop (Ocmp Ceq) tran_dst_regvar_mask inv_ss_regvar_mask)).
...
Fixpoint __C_will_dev_go_target_state (trans : list state_trans) (inv_ss : list sstate)
  ...
  | hd :: tl =>
                let now_ss := C_sstate_already_in_target hd iomem_p in
...
Definition C_eval_master_access (i : invariant) : Cminor.stmt :=
  Sifthenelse (C_is_valid_dev_access (i.(master_dev)).(regs))
    (Sifthenelse ((C_will_dev_go_target_state (i.(master_dev)).(trans)
...
Sifthenelse ( Ebinop (Ocmpu Cgt)(Evar regaccess_time) (Econst (Ointconst (i.(period2))))) C_return_allow
	(Sifthenelse ( Ebinop (Ocmpu Clt)(Evar regaccess_time) (Econst (Ointconst (i.(period1))))) C_return_allow
		( Sifthenelse (C_is_dev_access (i.(master_dev)).(dev_id)) (C_eval_master_access i) C_return_allow)).

Definition transl_invariant (i: invariant) : res Cminor.stmt :=
  match (i.(inv_binder)) with
  | uni_r => OK (transl_uni_r_binder i)
  | uni_s => OK Sskip
  | bidir => OK Sskip
  end.

Fixpoint transl_code (c: Ditiospec.code) : res Cminor.stmt :=
  match c with
...
Definition transl_funbody (f: Ditiospec.function) : res Cminor.stmt :=
  do b <- transl_code f;
  ...
  let regaccess_time_offset := Econst (Ointconst (Int.repr REGACCESS_TIME_OFFSET)) in
  let regaccess_time_p := Ebinop Oadd (Evar regaccess_p) regaccess_time_offset in
  let init_d := (Sassign regaccess_time (Eload Mint8unsigned regaccess_time_p)) in
  OK (Sseq init_a (Sseq init_b (Sseq init_c (Sseq init_d b)))).
(*  OK (Sseq init_a (Sseq init_b (Sseq init_c init_d))). (*(Sseq init_d b)))).*) *)
Definition transl_function (f: Ditiospec.function) : res Cminor.function :=
  do body <- transl_funbody f;
  let params := [ m_iomem_p; s_iomem_p; regaccess_p ] in
  let vars := [ regaccess_dev_id; regaccess_reg_off; regaccess_val; regaccess_time] in
  let stackspace := 10 * ditio_memwords in
  OK (Cminor.mkfunction signature_main params vars stackspace body).

Definition transl_fundef (fd: Ditiospec.fundef) :=
  match fd with
  | Internal f => do f' <- transl_function f; OK (Internal f')
  | External ef => Error (msg "no external function allowed")
  end.

Definition transl_program (p: Ditiospec.program) : res Cminor.program :=
